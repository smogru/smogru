    ...
    for(let i=0;i<pts.length;i++)
        for(let j=i+1;j<pts.length;j++){
            ...
            if(d<80){
                ...
                cx.strokeStyle=`rgba(56,189,248,${.025*(1-d/80)})`;cx.lineWidth=.4;cx.stroke()
            }
        }
}
function an(){cx.clearRect(0,0,cvs.width,cvs.height);pts.forEach(p=>{p.update();p.draw()});dl();requestAnimationFrame(an)}
an();
